    ...
    <div class="modal__overlay" tabindex="-1" data-micromodal-close>
        <div class="modal__container" role="dialog" aria-modal="true" aria-labelledby="rpressDateTime-title">
            ...
            <main class="modal__content" id="rpressDateTime-content">
                ...
                <h3><?= esc_html_e("Time preference", "restropress") ?></h3>
                <div class="bg-gray rpress-time-preference-wrap">
                            ...
                            if (is_array($store_timings)):
                                foreach ($store_timings as $key => $time):
                                    ...
                                    if (class_exists('RPRESS_SlotLimit')) {
                                        ?>
                                        <option value="<?php echo esc_attr($time); ?>" <?php if ($selected_time == $filtered_time || $asap_option == $filtered_time)
                                               echo 'selected'; ?>>
                                        ...
                                        ?>
                                        <option value="<?php echo esc_attr($value); ?>" <?php if ($selected_time == $filtered_time || $asap_option == $filtered_time)
                                               echo 'selected'; ?>>
                    ...
                    <div class="rp-col-lg-12 rp-col-md-12 rp-col-sm-12 rp-col-xs-12">
                        <?php echo $cookie_service ?>
                        <?php do_action('rpress_before_service_time', $cookie_service); ?>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button"
                        class="btn btn-default rpress-editaddress-cancel-btn <?php echo esc_attr($button_style); ?>"
                        data-dismiss="modal"><?php esc_html_e("Cancel", "restropress") ?></button>
                    <button type="submit"
                        class="rpress-editaddress-submit-btn <?php echo esc_attr($button_style); ?>"><span
                            class="rp-ajax-toggle-text"><?php esc_html_e("Update", "restropress") ?></span></button>
                </div>
            </main>
        </div>
    </div>
