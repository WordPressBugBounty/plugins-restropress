...
$service_type = apply_filters('rpress_current_service_type', $service_type);
...
if (empty($service_date)) {
    $service_date_raw = apply_filters("rpress_service_date_raw", $service_date, $service_type);
    ...
    if (!empty($service_date_raw)) {
     ...
     $service_date = date_i18n('F j', strtotime($service_date));
}
...
if (!rpress_is_store_open($service_type, $service_date)) {
    ...
    if ("delivery_and_pickup" === $service_type) {
        ?>
        <p> <?= rpress_store_closed_message($service_type) ?></p>
        <?php
    }
...
<p class="rpress_order-address-wrap">
    ...
    <a id="editDateTime"><?php esc_html_e("Edit", "restropress") ?></a>
</p>