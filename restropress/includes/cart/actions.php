...
function rpress_process_add_to_cart( $data ) {
	...
	if ( rpress_straight_to_checkout() && ! rpress_is_checkout() ) {
		...
		wp_safe_redirect( rpress_get_checkout_uri() . $url_parameters, 303 );
		rpress_die();
	} else {
		wp_safe_redirect( remove_query_arg( array( 'rpress_action', 'fooditem_id', 'rpress_options' ) ) ); rpress_die();
	}
}
...
function rpress_process_remove_from_cart( $data ) {
	...
	wp_safe_redirect( remove_query_arg( array( 'rpress_action', 'cart_item', 'nocache' ) ) ); rpress_die();
}
...
function rpress_process_remove_fee_from_cart( $data ) {
	...
	wp_safe_redirect( remove_query_arg( array( 'rpress_action', 'fee', 'nocache' ) ) ); rpress_die();
}
...
function rpress_process_collection_purchase( $data ) {
	...
	wp_safe_redirect( add_query_arg( 'added', '1', remove_query_arg( array( 'rpress_action', 'taxonomy', 'terms' ) ) ) );
	rpress_die();
}
...
function rpress_process_cart_save( $data ) {
	...
	if( ! $cart ) {
		wp_safe_redirect( rpress_get_checkout_uri() ); exit;
	}
}
