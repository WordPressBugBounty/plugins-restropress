						...
						<div id="side-sortables" class="meta-box-sortables ui-sortable">
								...
								<div class="inside">
									<div class="rpress-admin-box">
										...
										<div class="rpress-admin-box-inside">
											<p>
												<span
													class="label"><?php esc_html_e('Order Status:', 'restropress'); ?></span>
										...
										<div class="rpress-admin-box-inside">
											<p>
												<span
													class="label"><?php esc_html_e('Payment:', 'restropress'); ?></span>
												<select name="rpress-payment-status"
													class="medium-text rpress-payment-status">
													<?php foreach (rpress_get_payment_statuses() as $key => $status): ?>
														<option value="<?php echo esc_attr($key); ?>" <?php selected($payment->status, $key, true); ?>>
															<?php echo esc_html($status); ?>
														</option>
													<?php endforeach; ?>
												</select>
												...
												?>
												<span alt="f223" class="rpress-help-tip dashicons dashicons-editor-help"
													title="<?php echo esc_attr($status_help); ?>"></span>
											</p>
										</div>
								...
								<div id="rpress-order-resend-receipt" class="postbox rpress-order-data">
									<div class="inside">
										<div class="rpress-order-resend-receipt-box rpress-admin-box">
											...
											<a href="<?php echo esc_url(add_query_arg(array('rpress-action' => 'email_links', 'purchase_id' => $payment_id))); ?>"
												id="<?php if (count($customer->emails) > 1) {
													echo 'rpress-select-receipt-email';
												} else {
													echo 'rpress-resend-receipt';
												} ?>" class="button-secondary alignleft"><?php esc_html_e('Resend Receipt', 'restropress'); ?></a>
											<span alt="f223" class="rpress-help-tip dashicons dashicons-editor-help"
												title="<?php esc_html_e('<strong>Resend Receipt</strong>: This will send a new copy of the purchase receipt to the customer&#8217;s email address. If fooditem URLs are included in the receipt, new file fooditem URLs will also be included with the receipt.', 'restropress'); ?>"></span>
											<?php if (count($customer->emails) > 1): ?>
												...
												<div class="rpress-order-resend-receipt-addresses" style="display:none;">
													<select class="rpress-order-resend-receipt-email">
														<option value="">
															<?php esc_html_e(' -- select email --', 'restropress'); ?>
														</option>
														<?php foreach ($customer->emails as $email): ?>
															<option value="<?php echo urlencode(sanitize_email($email)); ?>">
							...
							<div id="rpress-payment-notes" class="postbox">
								...
								<div class="inside">
									<div id="rpress-payment-notes-inner">
										...
										if (!empty($notes)):
											...
											foreach ($notes as $note):
												echo wp_kses(
													rpress_get_payment_note_html($note, $payment_id),
													array(
														'div' => array(
															'class' => array(),
															'id' => array(),
									...
									<textarea name="rpress-payment-note" id="rpress-payment-note"
										class="large-text"></textarea>
									<p>
										<button id="rpress-add-payment-note" class="button button-secondary right"
											data-payment-id="<?php echo absint($payment_id); ?>"><?php esc_html_e('Add Note', 'restropress'); ?></button>
									</p>
									<div class="clear"></div>
								</div><!-- /.inside -->
					...
					<div id="postbox-container-2" class="postbox-container">
							...
							<div class="inside rpress-clearfix">
										...
										<div class="rpress-delivery-details">
											<p>
												...
												<?php if (!empty($service_date)):
													...
													echo esc_html(apply_filters('rpress_service_date_view', $service_date));
												endif; ?>
											</p>
										</div>
										<div class="rpress-delivery-details">
											<p class="rp-service-details">
												...
												<?php if (get_post_meta($payment_id, "rpress_dinein_table_id", true)): ?>
													...
													<?php $table_id = get_post_meta($payment_id, "rpress_dinein_table_id", true) ?>
								...
								<?php if (!empty($order_note)): ?>
									<div class="column-container customer-instructions">
										<h3><?php echo sprintf(esc_html__('%s instructions:', 'restropress'), esc_html(rpress_service_label($service_type))); ?>
										</h3>
						...
						if (is_array($cart_items)):
							...
							<div id="rpress-purchased-items"
								class="postbox rpress-edit-purchase-element <?php echo esc_attr($column_count); ?>">
									...
									<div class="row rpress-purchased-row">
										<div class="rpress-order-items-wrapper">
											<ul class="rpress-purchased-items-list-wrapper <?php echo esc_attr($key); ?>">
												<li class="fooditem">
													<span class="rpress-purchased-fooditem-actions actions">
														...
														<a href="" class="rpress-order-remove-fooditem rpress-delete"
															data-key="<?php echo esc_attr($key); ?>"><?php esc_html_e('&times;', 'restropress'); ?></a>
													</span>
												...
												<li class="price">
													<span class="rpress-payment-details-label-mobile">
														<?php printf(esc_html_x('%s Total Price', 'payment details purchased item total - mobile', 'restropress'), esc_html(rpress_get_label_singular())); ?>
													</span>
													<span
						...
						<?php endif; ?>
						<div
							class="postbox rpress-edit-purchase-element rp-add-update-elements <?php echo esc_attr($column_count); ?>">
							<div class="rpress-add-fooditem-to-purchase-header row header">
								<ul class="rpress-purchased-items-list-wrapper">
									<li class="fooditem">
										<?php printf(esc_html__('Add New %s', 'restropress'), esc_html(rpress_get_label_singular())); ?>
									</li>
							...
							<div class="rpress-add-fooditem-to-purchase aa inside">
								<ul>
									<li class="fooditem">
										<span class="rpress-payment-details-label-mobile">
											<?php printf(esc_html_x('Select %s To Add', 'payment details select item to add - mobile', 'restropress'), esc_html(rpress_get_label_singular())); ?>
										</span>
										<?php
										$allowed_html = array(
									...
									<li class="rpress-add-fooditem-to-purchase-actions actions">
										...
										<a href="" id="rpress-order-add-fooditem"
											class="button button-secondary"><?php printf(esc_html__('Add New %s', 'restropress'), esc_html(rpress_get_label_singular())); ?></a>
									</li>
								</ul>
											...
											<div class="data column-container">
												<div class="column">
													<p>
														...
														?>
														<strong
															class="order-data-address-line"><?php esc_html_e('Street Address Line 1:', 'restropress'); ?></strong><br />
													...
													<p>
														<strong
															class="order-data-address-line"><?php esc_html_e('Street Address Line 2:', 'restropress'); ?></strong><br />
												...
												<div class="column">
													<p>
														...
														?>
														<strong
															class="order-data-address-line"><?php echo esc_html__('City:', 'restropress'); ?></strong><br />
														<input type="text" name="rpress-payment-address[0][city]"
															value="<?php echo esc_attr($city); ?>" class="large-text" />
													</p>
													<p>
														<?php $zip = !empty($address['zip']) ? $address['zip'] : ''; ?>
														<strong
															class="order-data-address-line"><?php echo esc_html__('Zip / Postal Code:', 'restropress'); ?></strong><br />
														<input type="text" name="rpress-payment-address[0][zip]"
															value="<?php echo esc_attr($zip); ?>" class="large-text" />
													</p>
												</div>
												<div class="column">
													<?php
													...
													<p id="rpress-order-address-country-wrap">
														<strong
															class="order-data-address-line"><?php echo esc_html__('Country:','restropress'); ?></strong><br />
														<?php
														$allowed_html = array(
															'select' => array(
																'name' => true,
