    ...
    <div class="rp-addon rp-metabox">
      ...
      <div class="rp-metabox-content">
        <div class="addon-category">
          <select name="addons[<?php echo sanitize_key($key); ?>][category]" class="rp-input rp-addon-lists"
          ...
          <label class="input_max_allowed">
            ...
            <input type="number" min="1" name="addons[<?php echo sanitize_key($key); ?>][max_addons]"
          ...
          <label class="input_min_allowed">
            ...
            <input type="number" name="addons[<?php echo sanitize_key($key); ?>][min_addons]"
              ...
              <?php if (!rpress_has_variable_prices($post_id)): ?>
                ...
                <?php foreach ($get_addons as $get_addon):
                  ...
                  if (rpress_has_variable_prices($post_id)) {
                    ...
                    foreach (rpress_get_variable_prices($post_id) as $price) {
                      ...
                      <?php if ($count == 1): ?>
                        ...
                        <?php if ($count == 1) { ?>
                          <td class="rp-addon-select td_checkbox"><input type="checkbox"
                              value="<?php echo esc_attr($addon_item_id); ?>" id="<?php echo esc_attr($addon_slug); ?>"
                              name="addons[<?php echo sanitize_key($key); ?>][items][]" class="rp-checkbox" <?php echo esc_html($selected); ?> /></td>
                        <?php } else { ?>
                          <td class="td_checkbox">&nbsp;</td>
                        <?php } ?>
                        <td class="add_label"><label
                            for="<?php echo esc_attr($addon_slug); ?>"><?php echo (esc_html($addon_item_name)); ?></label></td>
                        <td class="variation_label"><label
                            for="<?php echo esc_attr($price['name']); ?>"><?php echo esc_html($price['name']); ?></label></td>
                        <td class="addon_price"><input class="addon-custom-price" type="number" step="any" min="0.00"
                            placeholder="0.00" value="<?php echo esc_attr(rpress_sanitize_amount($addon_price)); ?>"
                            name="addons[<?php echo esc_attr($key); ?>][prices][<?php echo esc_attr($addon_item_id); ?>][<?php echo esc_attr($price['name']); ?>]">
                        </td>

                        <td class="td_checkbox"><input type="checkbox" data-variation_name="<?php echo esc_attr($price['name']); ?>"
                            value="<?php echo esc_attr($addon_item_id . '|' . $price['name']); ?>"
                            id="<?php echo esc_attr($addon_slug); ?>" name="addons[<?php echo sanitize_key($key); ?>][default][]"
                            class="rps-checkbox" <?php echo esc_html($default_var_selected); ?> /></td>
                      </tr>
                      <?php $count++;
                    }
                    echo '</tbody>';
                  ...
                  <?php } else {
                    ...
                    <tr class="rp-child-addon" id="tag-<?php esc_html_e($addon_item_id) ?>">
                      <td class="rp-addon-select td_checkbox"><input type="checkbox"
                          value="<?php echo esc_attr($addon_item_id); ?>" id="<?php echo esc_attr($addon_slug); ?>"
                          name="addons[<?php echo sanitize_key($key); ?>][items][]" class="rp-checkbox" <?php echo esc_html($selected); ?> /></td>
                      <td class="add_label"><label
                      ...
                      <td class="addon_price"><input class="addon-custom-price" type="number" step="any" min="0.00"
                          placeholder="0.00" value="<?php echo esc_attr(rpress_sanitize_amount($addon_price)); ?>"
                          name="addons[<?php echo esc_attr($key); ?>][prices][<?php echo esc_attr($addon_item_id); ?>]"></td>

                      <td class="tds_checkbox"><input type="checkbox" value="<?php echo esc_attr($addon_item_id); ?>"
                          id="<?php echo esc_attr($addon_slug); ?>" name="addons[<?php echo sanitize_key($key); ?>][default][]"
                          class="rp-checkbox" <?php echo esc_attr($default_selected); ?> /></td>
...
<?php else: ?>
  ...
  <div class="rp-addon rp-metabox">
    ...
    <div class="rp-metabox-content">
      <div class="addon-category">
        <select name="addons[<?php echo sanitize_key($count); ?>][category]" class="rp-input rp-addon-items-list"
        ...
        <label class="cb_required">
          <input type="checkbox" name="addons[<?php echo sanitize_key($count); ?>][is_required]" value="yes" />Is
          Required?
          <span> | </span>
        </label>
      </div>
