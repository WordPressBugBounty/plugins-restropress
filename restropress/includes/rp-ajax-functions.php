    ...
    if ($cart_key !== '') {
        ...
        if (!empty($cart_contents['addon_items'])) {
            foreach ($cart_contents['addon_items'] as $key => $val) {
                ...
                $addon_quantity[$val['addon_id']] = $val['quantity'];
            }
        }
    }
    ob_start();
        ...
        if (is_array($prices) && !empty($prices)) {
            ...
            <div class="rp-variable-price-wrapper">
                ...
                foreach ($prices as $k => $price) {
                    ...
                    $price_option_amount = rpress_currency_filter(rpress_format_amount($price['amount']));
                    ?>
                    <div class="food-item-list active rd-option">
                        <label for="<?php echo esc_attr($price_option_slug); ?>" class="radio-container">
                            <input type="radio" name="price_options" id="<?php echo esc_attr($price_option_slug); ?>"
                                data-value="<?php echo esc_attr($price_option_slug . '|1|' . $price['amount'] . '|radio'); ?>"
                                value="<?php echo esc_attr($k); ?>" <?php echo esc_attr($is_first); ?>
                                class="rp-variable-price-option" />
                            <span class="control__indicator"></span>
                            <span class="rd-option-name"><?php echo esc_html($price_option); ?></span>
                        </label>
    ...
    if (isset($addon_ids) && is_array($addon_ids) && !empty($addon_ids)) { ?>
        <div class="rp-addons-data-wrapper">
            ...
            foreach ($addon_ids as $parent) {
                ...
                ?>
                <div class="addons-wrapper addons-wrapper-<?php echo esc_attr($parent); ?>"
                    data-id="<?php echo esc_attr($parent); ?>">
                        ...
                        <?php if ($is_required == 'yes'): ?>
                            <span class="rp-addon-required">
                                ...
                                printf(
                                    ...
                                    esc_html__('%s Required', 'restropress'),
                                    esc_html($min_addons)
                                );
                                ?>
                            </span>
                    ...
                    <input type="hidden" name="is_required" class="addon_is_required"
                        value="<?php echo esc_attr($is_required); ?>" />
                    ...
                    if (is_array($child_addons) && !empty($child_addons)) {
                        ...
                        foreach ($child_addons as $child_addon) {
                            ...
                            $qty = $addon_quantity[$child_addon_id] ?? 1;
                            ...
                            if (is_array($chosen_addons)):
                                if (is_array($prices) && !empty($prices)):
                                    foreach ($prices as $p_id => $price):
                                        ...
                                        ?>
                                        <div class="food-item-list list_<?php echo esc_attr($p_id); ?> <?php if ($p_id == $price_id) {
                                                echo 'active';
                                            } ?> <?php echo esc_attr($item_classes); ?>">
                                            <label for="<?php echo esc_attr($child_addon_slug); ?>_<?php echo esc_attr($p_id); ?>"
                                                class="<?php echo esc_attr($use_addon_like); ?>-container">
                                                <?php
                                                $is_selected = in_array($child_addon_id . '|' . $price['name'], $chosen_addons) ? 'checked' : '';
                                                ?>
                                                <input class="<?php echo esc_attr($is_selected); ?>"
                                                    data-type="<?php echo esc_attr($use_addon_like); ?>" type="<?php echo esc_attr($use_addon_like); ?>"
                                                    name="<?php echo esc_attr($child_addon_type_name); ?>"
                                                    id="<?php echo esc_attr($child_addon_slug); ?>_<?php echo esc_attr($p_id); ?>"
                                                    value="<?php echo esc_attr($child_addon . "|$qty|" . $get_addon_price . '|' . $use_addon_like); ?>"
                                                    <?php echo esc_attr($is_selected); ?> data-addon-checked="<?php echo esc_attr($is_selected); ?>">
                                            ...
                                            <div class="rpress-addon-quantity">
                                                <?php if ($get_addon_price >= 0): ?>
                                                    <span
                                                        class="cat_price total-addon-price">&nbsp;+&nbsp;<?php echo esc_html(rpress_currency_filter(rpress_format_amount($get_addon_price))); ?></span>

                                                    <span
                                                        class="cat_price total-addon-quantity"><?php apply_filters('rp_food_addon_column_extra_html', $child_addon_id, $get_addon_price, $addon_id, $child_addon_slug); ?></span> <?php
                                                endif; ?>
                                            </div>
                                        </div> <?php
                                    endforeach;
                                    ...
                                    <div class="food-item-list active <?php echo esc_attr($item_classes); ?>">
                                        <label for="<?php echo esc_attr($child_addon_slug); ?>"
                                            class="<?php echo esc_attr($use_addon_like); ?>-container">
                                            <?php $is_selected = in_array($child_addon_id, $chosen_addons) ? 'checked' : '';
                                            ?>
                                            <input data-type="<?php echo esc_attr($use_addon_like); ?>"
                                                type="<?php echo esc_attr($use_addon_like); ?>"
                                                name="<?php echo esc_attr($child_addon_type_name); ?>"
                                                id="<?php echo esc_attr($child_addon_slug); ?>"
                                                value="<?php echo esc_attr($child_addon . "|$qty|" . $get_addon_price . '|' . $use_addon_like); ?>"
                                        ...
                                        <?php if ($get_addon_price >= 0): ?>
                                            <div class="rpress-addon-quantity">
                                                <span
                                                    class="cat_price total-addon-price">&nbsp;+&nbsp;<?php echo esc_html(rpress_currency_filter(rpress_format_amount($get_addon_price))); ?></span>
                                                <span
                                                    class="cat_price total-addon-quantity"><?php apply_filters('rp_food_addon_column_extra_html', $child_addon_id, $get_addon_price, $addon_id, $child_addon_slug); ?></span>
                                            </div>
                                        <?php endif; ?>
            ...
            if (is_array($child_addons) && !empty($child_addons)) {
                foreach ($child_addons as $child_addon) {
                    ...
                    <option data-price="<?php echo esc_attr($addon_price); ?>" data-id="$child_addon"
                        value="<?php echo esc_attr($child_addon_name . '|' . $child_addon . '|' . $child_addon_price . '|1'); ?> ">
                        <?php echo esc_html($child_addon_name . ' (' . $addon_price . ') '); ?>
                    </option>
                    <?php
                }
            }
