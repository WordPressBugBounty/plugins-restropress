...
function getGrowthArrowSvg(isPositive) {
    if (isPositive) {
...
function renderGrowthBadge(selector, value) {
    var numericValue = parseFloat(value);
    var outputValue = isNaN(numericValue) ? 0 : numericValue;
    var percentageText = outputValue.toFixed(2) + "%";
    var percentageDiv = $(selector);

    percentageDiv.removeClass("rp-green rp-red");

    if (outputValue > 0) {
        percentageDiv.addClass("rp-green");
        percentageDiv.html(getGrowthArrowSvg(true) + "+" + percentageText);
        return;
    }

    if (outputValue < 0) {
        percentageDiv.addClass("rp-red");
        percentageDiv.html(getGrowthArrowSvg(false) + percentageText);
        return;
    }

    percentageDiv.addClass("rp-green");
    percentageDiv.text("0.00%");
}
function applyDashboardSummary(response) {
    ...
    renderGrowthBadge('#total-refund-percentage', response.refund_percentage);
    renderGrowthBadge('#total-sales-percentage', response.sales_percentage);
}
function callAjax(indx, selc) {
    ...
    if (selectedfilter !== "" && selectedfilter !== "custom") {
        ...
        var data = {
            ...
            date: selectedfilter,
            nonce: rpress_vars.selected_filter_nonce
        };
        $.ajax({
            url: rpress_vars.ajaxurl,
            type: 'POST',
            ...
            success: function (response) {
                applyDashboardSummary(response);
            },
